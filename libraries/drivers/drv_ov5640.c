...
static rt_err_t ov5640_read_id(struct rt_i2c_bus_device *bus)
{
    rt_uint8_t read_value[2];
    rt_uint16_t id = 0;
    read_reg(bus, OV5640_SENSOR_MIDH, 1, &read_value[0]);
    read_reg(bus, OV5640_SENSOR_MIDL, 1, &read_value[1]);
    id = ((rt_uint16_t)(read_value[0] << 8) & 0xFF00);
    id |= ((rt_uint16_t)(read_value[1]) & 0x00FF);

    if (id != OV5640_MID)
    {
        LOG_E("ov5640 init error, mid: 0x%04x", id);
        return RT_ERROR;
    }

    LOG_I("ov5640 read mid success, mid: 0x%04x", id);

    read_reg(bus, OV5640_SENSOR_PIDH, 1, &read_value[0]);
    read_reg(bus, OV5640_SENSOR_PIDL, 1, &read_value[1]);
    id = ((rt_uint16_t)(read_value[0] << 8) & 0xFF00);
    id |= ((rt_uint16_t)(read_value[1]) & 0x00FF);

    if (id != OV5640_PID)
    {
        LOG_E("ov5640 init error, pid: 0x%04x", id);
        return RT_ERROR;
    }

    LOG_I("ov5640 read hid success, pid: 0x%04x", id);
...
void ov5640_jpeg_mode(void)
{
    rt_uint16_t i=0;
    /* set yun422 mode */
    for (i = 0; i < (sizeof(ov5640_yuv422_reg_tbl) / 2); i++)
    {
        write_reg(i2c_bus, ov5640_yuv422_reg_tbl[i][0],ov5640_yuv422_reg_tbl[i][1]);
    }

    /* set jpeg mode */
    for(i=0;i<(sizeof(ov5640_jpeg_reg_tbl)/2);i++)
    {
        write_reg(i2c_bus, ov5640_jpeg_reg_tbl[i][0],ov5640_jpeg_reg_tbl[i][1]);
    }
}
...
{
    rt_uint16_t i=0;
    for (i = 0; i < (sizeof(ov5640_rgb565_reg_tbl) / 2); i++)
    {
        write_reg(i2c_bus, ov5640_rgb565_reg_tbl[i][0],ov5640_rgb565_reg_tbl[i][1]);
    }
}

/* set auto exposure */
void ov5640_set_auto_exposure(rt_uint8_t level)
{
    rt_uint8_t i = 0;
    rt_uint8_t *p = (rt_uint8_t*)OV5640_AUTOEXPOSURE_LEVEL[level];
    for (i = 0; i < 4; i++)
    {
 ...
 * 4: home
 * */
void ov5640_set_light_mode(rt_uint8_t mode)
{
 ...
 * 4: +2
 * */
void ov5640_set_color_saturation(rt_uint8_t sat)
{
 ...
 * 4: 2
 * */
void ov5640_set_brightness(rt_uint8_t bright)
{
  write_reg(i2c_bus, 0xff, 0x00);
  write_reg(i2c_bus, 0x7c, 0x00);
 ...
 * 4: 2
 * */
void ov5640_set_contrast(rt_uint8_t contrast)
{
 ...
 * 6: Retro
*/
void ov5640_set_special_effects(rt_uint8_t eft)
{
...
void ov5640_set_window_size(rt_uint16_t sx,rt_uint16_t sy,rt_uint16_t width,rt_uint16_t height)
{
    rt_uint16_t endx;
    rt_uint16_t endy;
    rt_uint8_t temp;
...
rt_uint8_t ov5640_set_image_out_size(rt_uint16_t width,rt_uint16_t height)
{
...
rt_uint8_t ov5640_set_image_window_size(rt_uint16_t offx, rt_uint16_t offy, rt_uint16_t width, rt_uint16_t height)
{
...
rt_uint8_t ov5640_set_image_size(rt_uint16_t width ,rt_uint16_t height)
{
   rt_uint8_t temp;
   write_reg(i2c_bus, 0xFF, 0x00);
   write_reg(i2c_bus, 0xE0, 0x04);
...
int rt_ov5640_init(void)
{
    ...
    if (result != RT_EOK)
    {
        LOG_E("ov5640 soft reset error!");
        return RT_ERROR;
    }
    rt_thread_delay(200);

    result = ov5640_read_id(i2c_bus);
    if (result != RT_EOK )
    {
        LOG_E("ov5640 read id error!");
        return RT_ERROR;
    }

    for (i = 0; i < sizeof(ov5640_svga_init_reg_tbl) / 2; i++)
    {
        write_reg(i2c_bus, ov5640_svga_init_reg_tbl[i][0], ov5640_svga_init_reg_tbl[i][1]);
    }
